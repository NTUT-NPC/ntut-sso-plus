/**
 * 此腳本改編自 gnehs 所撰寫的「北科網路教學評量自動填寫小精靈」
 * 來源：https://github.com/gnehs/userscripts/blob/main/ntut-autofill-assessment.user.js
 */
declare global {
    interface Array<T> {
        pickRandom(): T;
...
export default defineContentScript({
    matches: [
        'https://isms-nagios.ntut.edu.tw/tgrade/PortalTGrade.jsp*',
        'https://aps-stu.ntut.edu.tw/tgrade/PortalTGrade.jsp*'
    ],
    runAt: 'document_idle',
    world: 'MAIN',
    main() {
        ...
        function fill(confirm: boolean) {
            (document.querySelector(`input[type=CheckBox][name="dec"]`) as HTMLInputElement).checked = true
            if (confirm) {
                // 同意
                for (let ele of document.querySelectorAll(`tr:not([bgcolor="#CCFFCC"])>td:nth-child(2)>input`)) {
                    if ((ele as HTMLInputElement).value !== 'D') (ele as HTMLInputElement).checked = true;
                }
                for (let ele of document.querySelectorAll(`tr:not([bgcolor="#CCFFCC"])>td:nth-child(3)>input`)) {
                    if ((ele as HTMLInputElement).value !== 'D' && Math.random() > 0.6) (ele as HTMLInputElement).checked = true;
                }
                for (let ele of document.querySelectorAll(`textarea`)) {
                    (ele as HTMLTextAreaElement).value = ProsSentences.pickRandom()
                }
            } else {
                // 不同意
                ...
                for (let ele of document.querySelectorAll(`tr:not([bgcolor="#CCFFCC"])>td:nth-last-child(2)>input`)) {
                    if ((ele as HTMLInputElement).value !== 'D' && Math.random() > 0.35) (ele as HTMLInputElement).checked = true;
                }
                for (let ele of document.querySelectorAll(`textarea`)) {
